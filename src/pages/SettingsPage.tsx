...
function TestResultBadge({ result, testing }: { result: TestResult | null; testing: boolean }) {
  if (testing) {
    return (
  ...
  return (
    <div className={clsx(
      ...
      {result.ok ? <Wifi size={13} className="flex-shrink-0 mt-0.5" /> : <WifiOff size={13} className="flex-shrink-0 mt-0.5" />}
      <div>
        <div className="font-medium">{result.message}</div>
        {result.detail && <div className="text-[11px] opacity-70 mt-0.5">{result.detail}</div>}
...
function ModelRow({
  entry, index, total, onUpdate, onDelete, onMoveUp, onMoveDown,
}: {
  entry: ModelEntry; index: number; total: number;
  ...
  return (
    <div className={clsx(
      ...
      index === 0 ? 'bg-indigo-500/5 border-indigo-500/20' :
      index === 1 ? 'bg-violet-500/5 border-violet-500/15' :
      ...
      <span className={clsx(
        ...
        index === 1 ? 'bg-violet-500/10 text-violet-400' :
        'text-slate-600',
      )}>
        {index === 0 ? 'Default' : index === 1 ? 'Fallback' : `#${index + 1}`}
      </span>
...
export default function SettingsPage({
  ...
  const [settings, setSettings] = useState<AppSettings>(() => savedSnapshot.current);
  const [apiKeyCache, setApiKeyCache] = useState<Partial<Record<AIProvider, string>>>({});
  ...
  useEffect(() => {
    const dirty = JSON.stringify(settings) !== JSON.stringify(savedSnapshot.current);
    setIsDirty(dirty);
    onDirtyChange?.(dirty);
  ...
  const handleProviderChange = (provider: AIProvider) => {
    ...
    const updatedCache = { ...apiKeyCache, [settings.aiProvider]: settings.apiKey };
  ...
  const updateModel = (id: string, patch: Partial<ModelEntry>) =>
    setSettings(s => ({ ...s, modelEntries: s.modelEntries.map(m => m.id === id ? { ...m, ...patch } : m) }));
  ...
  const addModelById = (modelId: string) => {
    ...
    if (modelId === '__custom__') {
      setSettings(s => {
        const next = [...s.modelEntries, { id: uid(), label: '', modelId: '' }];
        return { ...s, modelEntries: next, defaultModelId: next[0]?.id ?? s.defaultModelId, fallbackModelId: next[1]?.id ?? s.fallbackModelId };
      });
      return;
    }
    // Look in fetched list first, then fall back to providerDefaults
    const fetched = fetchedModels?.find(m => m.modelId === modelId);
    const template = providerDefaults[settings.aiProvider].models.find(m => m.modelId === modelId);
    ...
    setSettings(s => {
      ...
      return {
        ...
        fallbackModelId: s.fallbackModelId || next[1]?.id || s.defaultModelId || '',
      };
    });
  };
  ...
  const handleFetchModels = async () => {
    ...
    setFetchModelsError(null);
    try {
      const result = await fetchModels(settings.aiProvider, settings.apiKey, settings.baseUrl);
      if (result.ok) {
  ...
  const handleTest = async () => {
    ...
    setTestResult(null);
    try {
      const result = await testConnection(settings.aiProvider, settings.apiKey, settings.baseUrl);
  ...
  const showBaseUrl = settings.aiProvider !== 'Gemini' && settings.aiProvider !== 'Claude';
    ...
    <div className="flex flex-col h-full bg-surface-0">
      ...
      <div className="px-6 pt-5 pb-4 border-b border-border flex-shrink-0 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Configure AI provider, models, OS adapter, and app preferences</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        <div className="max-w-2xl space-y-8">
          ...
          <section className="card p-5">
            ...
            <div className="mb-4">
              ...
              {settings.aiProvider === 'AIHoc' && (
                <div className="mt-2 flex items-start gap-2 text-[11px] text-slate-500 bg-surface-3 border border-border rounded-lg px-3 py-2">
                  ...
                  AIHoc is an AI gateway offering free open-source models (Llama, Mistral, Gemma) and premium models
                  (OpenAI, Gemini, Claude) under one API key. Model IDs follow{' '}
                  <code className="text-indigo-300 font-mono">provider/model-name</code> — you can type any model the gateway supports.
                </div>
              )}
            </div>
            ...
            {/* API Key */}
            <div className="mb-4">
              <FieldLabel hint={settings.aiProvider === 'Local Model' ? '(not required)' : undefined}>
                API Key
              </FieldLabel>
              <div className="relative">
                ...
                <input
                  ...
                  disabled={settings.aiProvider === 'Local Model'}
                  className="input-base w-full pl-9 pr-10 py-2 disabled:opacity-40"
                  placeholder={
                    settings.aiProvider === 'AIHoc'       ? 'aihoc-sk-…' :
            ...
            {showBaseUrl && (
              <div className="mb-4">
                <FieldLabel hint="(editable — supports Azure, proxies, self-hosted)">Base URL</FieldLabel>
                <div className="relative">
            ...
            <div className="space-y-2 mb-6">
              <button
                onClick={handleTest}
                disabled={testing || (!settings.apiKey && settings.aiProvider !== 'Local Model')}
            ...
            <div className="border-t border-border pt-5">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <div className="text-[11px] text-slate-600 mt-0.5">
                    Edit labels and model IDs freely — type any model name the provider accepts.
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                {/* Fetch models button */}
                <button
                  ...
                  disabled={fetchingModels || (!settings.apiKey && settings.aiProvider !== 'Local Model')}
                  title={!settings.apiKey && settings.aiProvider !== 'Local Model' ? 'Enter an API key first' : 'Fetch model list from provider'}
                  className="btn-ghost text-xs py-1 px-2 disabled:opacity-40"
                >
                  <RefreshCw size={12} className={fetchingModels ? 'animate-spin' : ''} />
                  {fetchingModels ? 'Fetching…' : fetchedModels ? `${fetchedModels.length} models fetched` : 'Fetch from API'}
                </button>
                ...
                <div className="relative">
                  <select
                    key={addPickerKey}
                    defaultValue=""
                    onChange={e => { if (e.target.value) addModelById(e.target.value); }}
                    className="input-base text-xs py-1 pl-2 pr-7 appearance-none cursor-pointer"
                  >
                    <option value="" disabled>+ Add model…</option>
                    {(fetchedModels ?? providerDefaults[settings.aiProvider].models)
                      .filter(m => !settings.modelEntries.some(e => e.modelId === m.modelId))
                      .map(m => (
                        <option key={m.modelId} value={m.modelId}>
                          {m.label !== m.modelId ? `${m.label} — ${m.modelId}` : m.modelId}
                        </option>
                      ))}
              ...
              {fetchModelsError && (
                <div className="flex items-center gap-1.5 text-[11px] text-red-400 mt-1">
                  <AlertCircle size={11} /> {fetchModelsError}
                </div>
              )}
              </div>
              ...
              {settings.modelEntries.length > 0 && (
                <div className="flex items-center gap-2 px-2 mb-1">
                  <div className="w-5" />
                  <div className="w-14 text-[10px] text-slate-600 uppercase tracking-widest">Priority</div>
                  <div className="flex-1 text-[10px] text-slate-600 uppercase tracking-widest">Label</div>
                  <div className="flex-[1.5] text-[10px] text-slate-600 uppercase tracking-widest">Model ID</div>
                  <div className="w-12" />
                  <div className="w-6" />
                </div>
              )}
              ...
              {settings.modelEntries.length === 0 ? (
                <div className="text-center py-6 text-xs text-slate-600 border border-dashed border-border rounded-lg">
                  No models yet. Click <strong>Add</strong> to create one.
                </div>
              ) : (
                <div className="space-y-1.5">
                  {settings.modelEntries.map((entry, index) => (
                    <ModelRow
                      key={entry.id}
                      entry={entry}
                      index={index}
                      total={settings.modelEntries.length}
                      onUpdate={patch => updateModel(entry.id, patch)}
                      onDelete={() => deleteModel(entry.id)}
                      onMoveUp={() => moveModel(entry.id, -1)}
                      onMoveDown={() => moveModel(entry.id, 1)}
                    />
                  ))}
                </div>
              )}
              ...
              {settings.modelEntries.length > 0 && (
                <div className="mt-2 text-[11px] text-slate-600 flex items-center gap-1.5">
                  ...
                  First row = Default, second row = Fallback. Drag with ↑↓ to reorder.
                </div>
              )}
          ...
          <section className="card p-5">
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              ...
              {(() => {
                ...
                const active = settings.adapterStatus === 'Win Use';
                return (
                  <button
                    onClick={() => valid && update({ adapterStatus: 'Win Use' })}
                    disabled={!valid}
                    title={!valid ? 'Win Use requires Windows' : undefined}
                    className={clsx(
                  ...
                  >
                    <div className="flex items-center gap-2 w-full">
                      <Monitor size={14} className={active && valid ? 'text-indigo-400' : 'text-slate-500'} />
                      <span className={clsx('text-xs font-semibold', active && valid ? 'text-slate-200' : 'text-slate-400')}>Win Use</span>
                      {active && valid && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-emerald-400" />}
                    </div>
                    <div className="text-[11px] text-slate-600 leading-snug">
                      Native Windows automation via UI Automation APIs.
                    </div>
                    {!valid && <div className="text-[10px] text-amber-500">Requires Windows</div>}
                  </button>
                );
              })()}
              ...
              {(() => {
                ...
                const active = settings.adapterStatus === 'Ghost OS';
                return (
                  <button
                    onClick={() => valid && update({ adapterStatus: 'Ghost OS' })}
                    disabled={!valid}
                    title={!valid ? 'Ghost OS requires macOS' : undefined}
                    className={clsx(
                      'flex flex-col items-start gap-1.5 p-3 rounded-lg border text-left transition-colors',
                      active && valid
                        ? 'bg-indigo-500/10 border-indigo-500/30'
                  ...
                  >
                    <div className="flex items-center gap-2 w-full">
                      <Monitor size={14} className={active && valid ? 'text-indigo-400' : 'text-slate-500'} />
                      <span className={clsx('text-xs font-semibold', active && valid ? 'text-slate-200' : 'text-slate-400')}>Ghost OS</span>
                      {active && valid && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-emerald-400" />}
                    </div>
                    <div className="text-[11px] text-slate-600 leading-snug">
                      macOS automation via Accessibility and AppleScript.
                    </div>
                    {!valid && <div className="text-[10px] text-amber-500">Requires macOS</div>}
                  </button>
                );
              })()}
            </div>
            ...
            <div className="flex items-center gap-3 p-3 bg-surface-3 border border-border rounded-lg mb-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-slate-200">{settings.adapterStatus}</div>
                <div className="text-xs text-slate-500">{settings.currentOS} · active adapter</div>
              </div>
              <div className="flex items-center gap-1.5">
                <span className={clsx('w-2 h-2 rounded-full', settings.adapterStatus !== 'Not Connected' ? 'bg-emerald-400' : 'bg-red-400')} />
                <span className={clsx('text-xs font-medium', settings.adapterStatus !== 'Not Connected' ? 'text-emerald-400' : 'text-red-400')}>
